...
// brief: defines a StorageBuffer struct;
// a StorageBuffer is, roughly, a deque of arrays. It is designed this way
// to minimize reallocs, so it would be a safe owner for pointers (eg.
// catacomb's ResourceManager uses StorageBuffers under the hood)
//
...
public:
    ...
    explicit StorageBuffer(Deleter deleter) : m_deleter(std::move(deleter)) {}
    
    // inserts an element & returns a SharedPtr to the inserted obj
    // need an rvalue as the StorageBuffer should always own its elements
...
private:
    ...
    // @brief: define a memory block for a StorageBuffer
    struct Block
    {
    public:
        Block()
